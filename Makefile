...
CHART_SYSTEM_K8S0 := cert-manager cilium ingress-nginx argocd cluster-issuer external-secrets metallb-operator metallb-config kyverno gitlab-operator
CHART_APPS_K8S0 := harbor gitlab
...
k8s0-system-%:
	...
	@helm dependency update --skip-refresh $(CHART_DIR_SYSTEM_K8S0)/$*

k8s0-apps-%:
	@echo "Packaging $* chart for k8s0 apps..."
	@helm dependency update --skip-refresh $(CHART_DIR_APPS_K8S0)/$*
